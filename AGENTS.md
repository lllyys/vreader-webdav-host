...
- **Codex review per PR**: every PR must pass `/codex-toolkit:audit-fix` (the audit → fix → verify loop) before `gh pr create`. Apply every High finding (no waivers); apply or explicitly waive every Medium finding in the PR body. Include the Codex thread ID. See `.claude/rules/30-codex-review.md`.
